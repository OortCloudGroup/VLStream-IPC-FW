...
/* 抛线程工具 */
static void createthread(void *priv, void *callback)
{
    pthread_t th;
    pthread_attr_t attr;
    int ret;
    ...
    /* 抛出线程 */
    ret = pthread_create(&th, &attr, callback, (void *)priv);
    if (ret != 0)
        printf("createthread failed !! %s\r\n", strerror(ret));
    /* attr destroy */
...
void fbmapCamera_read(CameraMap_Struct *cms)
{
    ...
    {
        /* 解析时间戳 */
        cms->timeStamp = buffer.timestamp.tv_sec * 1000 + buffer.timestamp.tv_usec / 1000;
 ...
 *  返回: 返回结构体指针,失败NULL
 */
FileMap_Struct *fileMap_open(const char *file, FileMap_Type type, int size)
{
    /* 参数检查 */
    if (!file)
    {
    ...
    case FMT_NEW:
        ...
        {
            uint8_t bytes = 0;
            for (int bytesCount = 0; bytesCount < size; bytesCount++) {
                write(fd, &bytes, 1);
            }
            /* 指针位置恢复到文件头 */
            lseek(fd, 0, SEEK_CUR);
    ...
    uint8_t *mem = (uint8_t *)mmap(0, size, prot, MAP_SHARED, fd, 0);
    if (!mem)
    {
    ...
    /* 返回参数 */
    FileMap_Struct *fms = (FileMap_Struct *)calloc(1, sizeof(FileMap_Struct));
    fms->mem = mem;
    fms->fd = fd;
    fms->size = size;
    fms->type = type;
    ...
    void (*callback)(CameraMap_Struct *, uint8_t *, int))
{
    ...
    /* 打开文件 */
    int fd = open(videoDev, O_RDWR | O_NONBLOCK);
    if (fd < 1)
    {
        fprintf(stderr, "fileMap_open: can't open %s \r\n", videoDev);
        return NULL;
    ...
    uint32_t default_fmt = 0;
    fmtdesc.index = 0;
    fmtdesc.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    while (ioctl(fd, VIDIOC_ENUM_FMT, &fmtdesc) == 0)
    {
        if (default_fmt == 0) {
            default_fmt = fmtdesc.pixelformat;
        }

        fmtdesc.index++;
        printf("support fmt: %4s, %s, ", (char *)&fmtdesc.pixelformat, fmtdesc.description);
    ...
    if (type) {
        ...
        fmt.fmt.pix.pixelformat = default_fmt;
    }
    ...
    printf("FMT: %d x %d - type %4s - size %d - fps %d\r\n",
           fmt.fmt.pix.width,
           fmt.fmt.pix.height,
    ...
    if (ioctl(fd, VIDIOC_EXPBUF, &expbuf) != 0) {
        fprintf(stderr, "ioctl VIDIOC_EXPBUF failed: %s\n", strerror(errno));
    }
...
void cameraMap_close(CameraMap_Struct *cms)
{
    ...
    {
        cms->runFlag = 2;
        for (int i = 0; i < 1000 && cms->runFlag == 2; i += 10) {
            usleep(10000);
        }
    }
    cms->runFlag = 0;
